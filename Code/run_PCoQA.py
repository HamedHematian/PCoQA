...
parser.add_argument("-m", "--model", type=str, help="Specify the model", required=True)
parser.add_argument("-p", "--pretrained_dataset", type=str, help="Specify the pretraining dataset", required=True)
parser.add_argument("-t", "--do_test", action="store_true", help="Specify whether to perform testing")
parser.add_argument("-n", "--hist_num", type=int, help="Specify whether to perform testing")
args = parser.parse_args()

if args.pretrained_dataset == 'quac' and args.model == 'parsbert':
  raise ValueError('Parsbert can not be used with QuAC dataset')
...
parsbert_model_id = 'HooshvareLab/bert-fa-zwnj-base'
xlmr_model_id = 'xlm-roberta-base'
if args.model == 'xlmr':
  model_path_or_name = xlmr_model_id
elif args.model == 'parsbert':
  model_path_or_name = parsbert_model_id
  
tokenizer = AutoTokenizer.from_pretrained(model_path_or_name)
model = AutoModel.from_pretrained(model_path_or_name)

print(f'Using {args.model}, {model_path_or_name} Loaded')
...
def make_features(data_type):
  ...
  max_history_to_consider = args.hist_num
  ...
  for file_index, filename in enumerate(example_files):
    ...
    for example in tqdm(examples, leave=False, position=0):
      ...
      extra_token = 2 if args.model == 'xlmr' else 0
      
      text_tokens = tokenizer(
          concatenated_question,
          example.cleaned_context,
          max_length=model.config.max_position_embeddings - extra_token,
          padding='max_length',
...
is_pretrained = args.pretrained_dataset

parsbert_parsquad = '1-QnT-eaYQ_fKFyCx-mjAKZDwIJBTa7fJ'
xlmr_parsquad = '1-carGHsxRtjl23CEOOaVCQzOPLMqgBKA'
xlmr_quac = '1-zKrKtyIGtJhzBlSw56_z8ZfjDcJlkB6'

if is_pretrained == 'parsquad' and args.model == 'parsbert':
  gdown.download(id=parsbert_parsquad)
  target_dir = 'Parsbert_ParSQuAD'
  pretrained_weights = torch.load('checkpoint_3_0_0')
elif is_pretrained == 'parsquad' and args.model == 'xlmr':
  gdown.download(id=xlmr_parsquad)
  target_dir = 'XLMR_ParSQuAD'
  pretrained_weights = torch.load('checkpoint_3_0_0')
elif is_pretrained == 'quac' and args.model == 'xlmr':
  gdown.download(id=xlmr_quac)
  target_dir = 'XLMR_QuAC'
  pretrained_weights = torch.load('checkpoint_3_0_0')
elif is_pretrained == 'none':
  target_dir = f'{args.model}_None'



hist_num_str = args.hist_num

drive_prefix = f'{args.model}_FineTune_{target_dir}_{hist_num_str}/'
...
if is_pretrained in ['quac', 'parsquad']:
...
checkpoint_config = torch.load(best_eval_checkpoint)
