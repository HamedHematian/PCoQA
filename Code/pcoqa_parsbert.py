...
make_examples(train_data, 'train', 100000)
make_examples(eval_data, 'eval', 100000)
make_examples(test_data, 'test', 100000)
make_features('train')
make_features('eval')
make_features('test')
...
if is_pretrained in ['parsquad', 'quac']:
  pretrain_parsquad_id = '1-QnT-eaYQ_fKFyCx-mjAKZDwIJBTa7fJ'
  pretrain_quac_id = '1-fKF2wK3lnuIIAjOoe-jCZJ0jOIWT2L6'
...
drive_prefix = f'drive/MyDrive/Parsbert_FineTune_{target_dir}_{hist_num_str}/'
...
optimizer = AdamW(model_p.parameters(), lr=lr, betas=(beta_1,beta_2), eps=eps)
...
checkpoint_config = torch.load(f'drive/MyDrive/Parsbert_FineTune_{target_dir}_2/Checkpoint/best_model_2.pt')
